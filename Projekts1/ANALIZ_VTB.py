...
PDF_REPORT_FILENAME = "OUTPUT_Analiz_portfolio.pdf"
_PDF_FONT_CACHE: Optional[str] = None
PDF_FONT_NAME = "AnalizVTBReportFont"
SUMMARY_SNAPSHOT_PATH = ROOT_DIR / "out" / "analysis_summary.json"
REPORT_MESSAGES_PATH = ROOT_DIR / "out" / "report_messages.json"
...
def ensure_pdf_font() -> str:
    """
    Register a font capable of displaying Cyrillic characters for PDF export.
    Falls back to Helvetica if no better option is available.
    """
    ...
    env_font = os.environ.get("ANALIZ_VTB_PDF_FONT")
    if env_font:
    ...
    font_candidates.append(ROOT_DIR / "fonts" / "DejaVuSans.ttf")
...
def build_overall_summary_snapshot(report_context: dict) -> dict:
    ...
    for line in build_overall_message(report_context).splitlines():
        if line.strip() == '-----------------------------------------------------------------':
            break
        header_lines.append(line)
    metrics = {
        'total_current_value_fmt': overall.get('total_current_value_fmt', '0.00'),
        'kopilka_current_value_fmt': (kopilka or {}).get('current_value_fmt', '0.00'),
        'total_working_capital_fmt': overall.get('total_working_capital_fmt', '0.00'),
        'absolute_profit_fmt': overall.get('total_absolute_profit_value_fmt', '0.00'),
        'capital_growth_percent_fmt': overall.get('average_absolute_profit_percent_fmt', '0.00%'),
        'yield_percent_fmt': overall.get('average_yield_fmt', overall.get('average_portfolio_yield_fmt', '0.00%')),
    }
    return {
...
def save_report_artifacts(
    ...
    """Сохраняет метрики и текстовые сообщения для дальнейшего использования в GUI."""

    snapshot = build_overall_summary_snapshot(report_context)
    SUMMARY_SNAPSHOT_PATH.parent.mkdir(parents=True, exist_ok=True)
    SUMMARY_SNAPSHOT_PATH.write_text(
        json.dumps(snapshot, ensure_ascii=False, indent=2),
        encoding='utf-8',
    )
    REPORT_MESSAGES_PATH.write_text(
        json.dumps({'messages': report_messages}, ensure_ascii=False, indent=2),
        encoding='utf-8',
    )
...
def format_security_list(items: List[dict]) -> str:
    ...
    for entry in items:
        ...
        secid = entry.get("SECID") or ""
        identifier = f"ISIN: {isin}"
        if secid and secid != isin:
            identifier += f", SECID: {secid}"
...
def build_overall_message(report_context: dict) -> str:
    ...
    portfolios_index = report_context.get("portfolios_index") or {}
    kopilka = portfolios_index.get("1UJ8S KOPILKA")
    defaults = report_context.get("defaults") or {}
    ...
    total_current_value = overall.get("total_current_value_fmt", "0.00")
    total_working = overall.get("total_working_capital_fmt", "0.00")
    total_profit = overall.get("total_absolute_profit_value_fmt", "0.00")
    profit_percent = overall.get("average_absolute_profit_percent_fmt", "0.00%")
    avg_yield = overall.get("average_yield_fmt", "0.00%")
    kopilka_value = kopilka.get("current_value_fmt", "0.00") if kopilka else "0.00"

    def filter_defaults(entries: List[dict], *, technical: bool = False) -> List[dict]:
        filtered: List[dict] = []
        for item in entries:
            if (item.get("Portfolio") or "").strip() != "124JAV IIS-3":
                continue
            has_technical = db_flag(item.get("HASTECHNICALDEFAULT"))
            has_default = db_flag(item.get("HASDEFAULT")) or (
                (item.get("is_traded") or "").strip().upper() == "ДЕФОЛТ"
            )
            if technical and has_technical:
                filtered.append(item)
            elif not technical and has_default and not has_technical:
                filtered.append(item)
        return filtered
    ...
    lines = [
        ...
        f"{total_current_value}₽ из которых {kopilka_value}₽ на счету \"KOPILKA\"",
        "",
        f"- 💰 **Работающие деньги:** {total_working} ₽",
        f"- ✅ **Абсолютная прибыль:** {total_profit} ₽",
        f"- 💸 **Прирост капитала:** {profit_percent}",
        f"- 🏆 **Доходность:** {avg_yield}",
        "-----------------------------------------------------------------",
        " ⛔️ ДЕФОЛТНЫЕ АКТИВЫ📛 Портфель \"IIS-3\"",
        "❌ Дефолт",
        format_security_list(filter_defaults(defaults.get("default", []))),
        "⁉️ Тех.Дефолт",
        format_security_list(filter_defaults(defaults.get("technical", []), technical=True)),
        "-----------------------------------------------------------------",
        "🔹🔹🔹 Портфель \"KOPILKA\" 🔹🔹🔹",
    ]
    ...
    if kopilka:
        lines.extend(
            [
                ...
                f"  💵 **Средняя купонная доходность:** {kopilka['average_yield_fmt']}",
            ]
        )
    else:
...
def build_portfolio_message(report_context: dict, portfolio_code: str) -> str:
    ...
    lines = [
        ...
        f"  💵 **Средняя купонная доходность:** {entry['average_coupon_yield_fmt']}",
        "",
...
def send_telegram_report(
    ...
    base_messages = messages or build_report_messages(report_context)
...
def export_report_to_pdf(
    ...
    if SimpleDocTemplate is None or Paragraph is None:
        LOGGER.warning("ReportLab is not available. Skipping PDF export.")
        return None

    base_messages = messages or build_report_messages(report_context)
    if not base_messages:
        LOGGER.warning("No report messages to convert into PDF.")
        return None

    pdf_path = ROOT_DIR / "out" / PDF_REPORT_FILENAME
    if pdf_path.exists():
        try:
            pdf_path.unlink()
        except PermissionError as exc:
            raise SystemExit(
                f"Cannot overwrite {pdf_path}. Close the file and retry."
            ) from exc

    creation_date = report_context.get("creation_date", date.today().isoformat())
    ...
    doc = SimpleDocTemplate(
        str(pdf_path),
        pagesize=A4 or (595.27, 841.89),
        leftMargin=18 * mm if mm else 36,
        rightMargin=18 * mm if mm else 36,
        topMargin=18 * mm if mm else 36,
        bottomMargin=20 * mm if mm else 40,
    )

    elements: List[object] = [
        Paragraph(
            escape(f"Отчет ВТБ по портфелю на {creation_date}"),
            title_style,
        ),
        Spacer(1, 12),
    ]

    for idx, message in enumerate(base_messages, start=1):
        elements.append(Paragraph(escape(f"Блок {idx}"), section_style))
        formatted_message = escape(message).replace("\n", "<br/>")
        elements.append(Paragraph(formatted_message, body_style))
...
def main() -> None:
    ...
    report_messages = build_report_messages(report_context)
    save_report_artifacts(report_context, report_messages)
    export_to_excel(
        summary,
        overall_metrics,
        distribution_rows,
    ...
    pdf_path = export_report_to_pdf(report_context, report_messages)
    ...
    print("Для отправки отчета используйте кнопки в GUI.")
